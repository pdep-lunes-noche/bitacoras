...
toArray :: a -> [a]
toArray a = [a]
dave = UnBarbaro "Dave" 100 ["tejer","escribirPoesia"] [ardilla, espada 10, cerveza, megafonoBarbarico]
...
usarTodosLosObjetos :: Barbaro -> Barbaro
usarTodosLosObjetos barbaro = foldr ($) barbaro (objetos barbaro)

usarTodosLosObjetos' :: Barbaro -> Barbaro
usarTodosLosObjetos' barbaro = foldl1 (.) (objetos barbaro) barbaro

usarTodosLosObjetos'' :: Barbaro -> Barbaro
usarTodosLosObjetos'' barbaro = foldl (flip ($)) barbaro (objetos barbaro)

aplicarObjeto :: Barbaro -> Objeto -> Barbaro
aplicarObjeto barbaro objeto = objeto barbaro

usarTodosLosObjetos''' :: Barbaro -> Barbaro
usarTodosLosObjetos''' barbaro = foldl aplicarObjeto barbaro (objetos barbaro)
...
miPropioMap :: (a -> b) -> [a] -> [b]
miPropioMap _ [] = []
miPropioMap funcion (x:xs) = funcion x : miPropioMap funcion xs

fibonacci :: Number -> Number
fibonacci 1 = 1
fibonacci 0 = 0
fibonacci numero = numero + fibonacci (numero - 1)

-- Los bárbaros son una raza muy orgullosa, 
-- tanto que quieren saber como son sus descendientes
-- El descendiente de un bárbaro comparte su nombre
-- y un asterisco por cada generacion

-- Dave , Dave*, Dave**, etc
-- Además utiliza todos sus objetos sobre si mismo
-- Las habilidades sin repetidos 


-- Definir descendientes que dado un barbaro
-- nos da sus infinitos descendientes

descendientes :: Barbaro -> [Barbaro]
descendientes barbaro = descendiente barbaro : descendientes (descendiente barbaro)

descendiente :: Barbaro -> Barbaro
descendiente = habilidadesSinRepetidos . obtenerNombreDescendiente . usarTodosLosObjetos

obtenerNombreDescendiente :: Barbaro -> Barbaro
obtenerNombreDescendiente barbaro = barbaro{nombre = (++"*") . nombre $ barbaro}

habilidadesSinRepetidos :: Barbaro -> Barbaro
habilidadesSinRepetidos barbaro = barbaro{habilidades = sinRepetidos.habilidades $ barbaro}
